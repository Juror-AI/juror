...
repository-owned routing or source. The planner can search and read bounded text from the sealed
checkout for this purpose, but cannot execute repository code, follow symbolic links, or inspect
paths outside that checkout. The broker continues to reject navigation outside the allowed origins,
and the QA agent must not navigate to API, sign-out, or state-changing endpoints.
