...
| Model subprocesses | Shell execution, filesystem escape, persistence, cross-juror leakage | Private temporary homes, kernel filesystem restrictions for Codex, path confinement for the generic harness, separate scratch directories; QA exposes only bounded literal search and line reads of allowlisted source/documentation files in its sealed checkout, excluding sensitive filenames and secret-shaped values and rejecting traversal, VCS metadata, and symbolic links | Sandboxing differs by harness and operating system; a harness vulnerability may weaken it |
