...
const repositorySettingsSchema = z.object({
  ...
  secretHeaders: z.array(z.object({ name: z.string().regex(/^(?:X-[!#$%&'*+\-.^_`|~0-9A-Za-z]+|CF-Access-Client-(?:Id|Secret))$/i), value: z.string().min(1).max(4096), origins: z.array(z.string().url()).min(1).max(20) })).max(20).optional(),
  resetHook: z.object({ url: z.string().url(), method: z.enum(['POST', 'PUT', 'PATCH', 'DELETE']), secretHeaders: z.array(z.object({ name: z.string().regex(/^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/), value: z.string().min(1).max(4096), format: z.enum(['bearer', 'raw']) })).max(20), expectedStatuses: z.array(z.number().int().min(200).max(499)).min(1), timeoutSeconds: z.number().int().min(1).max(60).optional() }).nullable().optional(),
