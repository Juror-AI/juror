...
async function upsertRepository(env: Env, installationId: number, repository: JsonObject, refreshActionDetection = false): Promise<{ repositoryId: string; workspaceId: string }> {
  ...
  const timestamp = now();
  await env.DB.prepare(`INSERT INTO repository (id, workspace_id, github_repository_id, owner, name, full_name, default_branch, is_private, archived, github_access_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ON CONFLICT(github_repository_id) DO UPDATE SET owner = excluded.owner, name = excluded.name, full_name = excluded.full_name, default_branch = excluded.default_branch, is_private = excluded.is_private, archived = excluded.archived, github_access_state = 'active', updated_at = excluded.updated_at`)
    .bind(repositoryId, workspace.id, repository.id, repository.owner.login, repository.name, repository.full_name, repository.default_branch ?? 'main', repository.private ? 1 : 0, repository.archived ? 1 : 0, timestamp, timestamp).run();
...
export async function createRun(env: Env, input: { kind: 'review' | 'qa'; identity: string; workspaceId: string; repositoryId: string; pullRequestId: string; prNumber: number; sha: string }): Promise<string | null> {
  ...
  const reservation = await env.DB.prepare(`INSERT OR IGNORE INTO run (id, identity, workspace_id, repository_id, pull_request_id, kind, status, phase, pr_number, revision_sha, reserved_micro_usd, created_at, updated_at)
    ...
    .bind(runId, input.identity, input.repositoryId, input.pullRequestId, input.kind, input.prNumber, input.sha, estimate, timestamp, timestamp, input.workspaceId, estimate, periodStart.toISOString(), estimate).run();
  ...
  if (!allowed) {
    ...
    const blocked = await env.DB.prepare(`INSERT OR IGNORE INTO run (id, identity, workspace_id, repository_id, pull_request_id, kind, status, phase, pr_number, revision_sha, reserved_micro_usd, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'blocked', 'billing', ?, ?, 0, ?, ?)`)
      .bind(runId, input.identity, input.workspaceId, input.repositoryId, input.pullRequestId, input.kind, input.prNumber, input.sha, timestamp, timestamp).run();
    if (!blocked.meta.changes) return null;
  }
  ...
  if (input.kind === 'qa') {
    const active = await env.DB.prepare(`SELECT COUNT(*) AS count FROM run WHERE repository_id = ? AND kind = 'qa' AND id != ? AND status IN ('queued', 'running') AND workflow_instance_id IS NOT NULL`).bind(input.repositoryId, runId).first<{ count: number }>();
    if ((active?.count ?? 0) > 0) {
      await appendRunEvent(env, runId, 'queued', 'pending', 'Queued behind an active repository QA run to protect the staging identity.');
      return runId;
    }
  }
  const workflow = input.kind === 'review' ? env.REVIEW_WORKFLOW : env.QA_WORKFLOW;
  const workflowClaim = await env.DB.prepare(`UPDATE run SET workflow_instance_id = ?, updated_at = ? WHERE id = ? AND status = 'queued'`).bind(runId, now(), runId).run();
  if (!workflowClaim.meta.changes) return runId;
...
export async function processGitHubWebhook(env: Env, eventName: string, payload: JsonObject): Promise<void> {
  if (eventName === 'installation') {
    ...
    else if (payload.action === 'deleted') await env.DB.batch([
      env.DB.prepare(`UPDATE installation SET suspended_at = ?, updated_at = ? WHERE github_installation_id = ?`).bind(now(), now(), payload.installation.id),
      env.DB.prepare(`UPDATE repository SET github_access_state = 'removed', updated_at = ? WHERE workspace_id = (SELECT workspace_id FROM installation WHERE github_installation_id = ?)`).bind(now(), payload.installation.id),
    ]);
    else if (payload.action === 'suspend') await env.DB.batch([
      env.DB.prepare(`UPDATE installation SET suspended_at = ?, updated_at = ? WHERE github_installation_id = ?`).bind(payload.installation.suspended_at ?? now(), now(), payload.installation.id),
      env.DB.prepare(`UPDATE repository SET github_access_state = 'suspended', updated_at = ? WHERE workspace_id = (SELECT workspace_id FROM installation WHERE github_installation_id = ?)`).bind(now(), payload.installation.id),
    ]);
    return;
  }
  if (eventName === 'installation_repositories') {
    for (const repository of payload.repositories_added ?? []) await upsertRepository(env, payload.installation.id, repository, true);
