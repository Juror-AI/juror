...
export async function startNextRepositoryQa(env: Env, completedRunId: string): Promise<boolean> {
  const next = await env.DB.prepare(`SELECT queued.id FROM run current JOIN run queued ON queued.repository_id = current.repository_id JOIN repository_settings settings ON settings.repository_id = queued.repository_id JOIN repository ON repository.id = queued.repository_id WHERE current.id = ? AND repository.github_access_state = 'active' AND settings.execution_mode = 'cloud' AND settings.qa_enabled = 1 AND settings.qa_security_ready = 1 AND queued.kind = 'qa' AND queued.status = 'queued' AND queued.workflow_instance_id IS NULL AND NOT EXISTS (SELECT 1 FROM run active WHERE active.repository_id = queued.repository_id AND active.kind = 'qa' AND active.id != queued.id AND active.status IN ('queued', 'running') AND active.workflow_instance_id IS NOT NULL) ORDER BY queued.created_at, queued.id LIMIT 1`)
...
export async function sweepQueuedQaAdmissions(env: Env): Promise<number> {
  const candidates = await env.DB.prepare(`SELECT queued.id FROM run queued JOIN repository_settings settings ON settings.repository_id = queued.repository_id JOIN repository ON repository.id = queued.repository_id WHERE repository.github_access_state = 'active' AND settings.execution_mode = 'cloud' AND settings.qa_enabled = 1 AND settings.qa_security_ready = 1 AND queued.kind = 'qa' AND queued.status = 'queued' AND queued.workflow_instance_id IS NULL AND NOT EXISTS (SELECT 1 FROM run active WHERE active.repository_id = queued.repository_id AND active.kind = 'qa' AND active.id != queued.id AND active.status IN ('queued', 'running') AND active.workflow_instance_id IS NOT NULL) AND NOT EXISTS (SELECT 1 FROM run earlier WHERE earlier.repository_id = queued.repository_id AND earlier.kind = 'qa' AND earlier.status = 'queued' AND earlier.workflow_instance_id IS NULL AND (earlier.created_at < queued.created_at OR (earlier.created_at = queued.created_at AND earlier.id < queued.id))) ORDER BY queued.created_at, queued.id LIMIT 50`)
    .all<{ id: string }>();
