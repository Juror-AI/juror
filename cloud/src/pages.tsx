...
function RepositoryRow({ repository, onSaved, presetReadiness }: { repository: RepositoryItem; onSaved: () => void; presetReadiness?: Record<ReviewPresetId, boolean> }) {
  ...
  const [executionMode, setExecutionMode] = useState(repository.executionMode);
  ...
  const [actionDisabled, setActionDisabled] = useState(!repository.actionDetected);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const reset = () => { setReviewEnabled(repository.reviewEnabled); setQaEnabled(repository.qaEnabled); setExecutionMode(repository.executionMode); setPreset(repository.reviewPreset); setPublishMode(repository.publishMode); setSeverityFloor(repository.severityFloor); setQaTarget(repository.qaTarget ?? ''); setAllowedOrigins(repository.allowedOrigins.join('\n')); setSessionUrl(''); setSessionTargetOrigin(repository.qaTarget ? new URL(repository.qaTarget).origin : ''); setSessionSecret(''); setSecretHeaders(''); setResetUrl(''); setResetHeaders(''); setScreenshotEvidence(repository.evidencePolicy.screenshot); setTraceEvidence(repository.evidencePolicy.trace); setVideoEvidence(repository.evidencePolicy.video); setActionDisabled(!repository.actionDetected); setError(null); };
  const save = async () => {
    ...
    try {
      ...
      const body: Record<string, unknown> = { executionMode, confirmActionDisabled: actionDisabled, reviewEnabled, reviewPreset: preset, publishMode, severityFloor, qaEnabled, qaTarget: qaTarget || null, allowedOrigins: allowedOrigins.split(/\s+/).filter(Boolean), evidencePolicy: { screenshot: screenshotEvidence, trace: traceEvidence, video: videoEvidence } };
  ...
  return <Card className={cn('repository-card', expanded && 'expanded')}>
    ...
    {expanded && <div className="repository-config">
      {repository.actionDetected && <div className="warning-callout compact-callout"><CircleAlert size={16} /><div><strong>Juror Action workflow detected</strong><p>Choose Action or Cloud mode before enabling hosted runs to prevent duplicates.</p></div><a className="button button-secondary button-small" href={`https://github.com/${repository.fullName}/tree/${repository.defaultBranch}/.github/workflows`} target="_blank" rel="noreferrer">Review workflow</a></div>}
